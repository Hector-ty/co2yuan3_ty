...
5. 在你的浏览器中输入你的服务器对应的 IP 地址并登录 RAGFlow。
   > 上面这个例子中，**本项目已将 RAGFlow Web 映射到主机 8082 端口**（避免与主项目 8080 冲突），请使用 http://IP_OF_YOUR_MACHINE:8082 访问。若使用默认配置（80:80），则无需输入端口。
...
如需更新 HTTP 服务端口，可在 [docker-compose.yml](./docker/docker-compose.yml) 中修改端口映射。**本项目当前为 `8082:80`**（主机 8082，避免与主项目 8080 冲突）；若使用默认则为主机 `80:80`，可改为 `<YOUR_SERVING_PORT>:80`。
